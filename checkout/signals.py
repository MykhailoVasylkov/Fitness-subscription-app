    ...
    if not created:
        ...
        if instance.status == 'shipped':
            ...
            subject = render_to_string(
                (
                    ...
                    'shipped_order_confirmation_email_subject.txt'
                ),
                {'order': instance}
            )
            body = render_to_string(
                (
                    'checkout/confirmation_emails/'
                    'shipped_order_confirmation_email_body.txt'
                ),
                {
                    'order': instance,
        ...
        if instance.status == 'completed':
            ...
            subject = render_to_string(
                (
                    'checkout/confirmation_emails/'
                    'completed_order_confirmation_email_subject.txt'
                ),
                {'order': instance})
            body = render_to_string(
                (
                    'checkout/confirmation_emails/'
                    'completed_order_confirmation_email_body.txt'
                ),
                {
                    'order': instance,
                    'contact_email': settings.DEFAULT_FROM_EMAIL,
