...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        print(f"save_info: {intent.metadata}")
        pid = intent.id
        bag_type = intent.metadata.bag_type
        bag = intent.metadata.product_bag if bag_type == 'product' else intent.metadata.plan_bag
        print(f"Plan Bag: {json.dumps(bag, indent=4)}")
            ...
            while attempt <= 5:
                try:
                    subscription = Subscription.objects.get(
                        ...
                        email__iexact=billing_details.email,
                        subscription_total=grand_total,
                        original_bag=bag,
                        stripe_pid=pid,
                    )
