...
def index(request):
    """
    A view to return the index page and collect
    subscription emails with confirmation token.
    Used Chat-GPT
    """
        ...
        if form.is_valid():
            ...
            confirmation_link = request.build_absolute_uri(
                reverse(
                    ...
                    args=[str(subscriber.confirmation_token)]
                )
            )
            ...
            html_content = render_to_string(
                "emails/confirm_email_subscription.html", context
            )
            text_content = (
                f"Please confirm your subscription"
                f" by visiting: {confirmation_link}"
            )
